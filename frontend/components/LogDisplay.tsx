  ...
  const getStatus = () => {
    ...
    if (logs.length > 0) {
      ...
      if (lastLog.includes("[ERROR]")) {
        return <span className="text-red-600 font-bold">Finished with an error.</span>;
      }
    ...
    <div className="rounded-lg border border-gray-300 bg-white/50 shadow-sm transition-all duration-300 w-full">
      ...
      {isOpen && (
        ...
        >
          {logs.length > 0 ? (
            logs.map((log, index) => (
              <p key={index} className="whitespace-pre-wrap break-words text-xs leading-relaxed hover:bg-gray-200">
                {log}
              </p>
            ))
          ) : (
